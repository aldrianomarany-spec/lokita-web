  ...
  return (
    <Overlay onClose={closeSell}>
      <div onClick={stop} style={{ background: '#FFFFFF', borderRadius: 0, padding: '30px 32px', width: '100%', maxWidth: 500, animation: 'lok-pop .26s cubic-bezier(.2,.8,.3,1) both', boxShadow: '0 40px 90px -20px rgba(18,19,21,.5)', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <p style={{ fontSize: 13, color: '#8B8B86', fontWeight: 600, margin: '0 0 18px' }}>Takes under a minute. Post it, drop it at the Security Post, done.</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div onClick={toggleBundle} style={{ display: 'flex', alignItems: 'center', gap: 12, background: '#F5F5F3', border: '1px solid #D8D8D4', borderRadius: 0, padding: '12px 14px', cursor: 'pointer' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: '#8B8B86', fontWeight: 500 }}>Clear your whole room in one deal</div>
            </div>
          </div>
        ...
        <button disabled={busy} className="lok-btn" onClick={() => submitListing(photos)} style={{ width: '100%', border: 'none', background: listBg, color: '#F7F3EA', fontFamily: 'inherit', fontWeight: 700, fontSize: 14.5, padding: 14, borderRadius: 0, cursor: busy ? 'default' : 'pointer', marginTop: 18, transition: 'background .2s ease', boxShadow: '0 8px 20px -8px rgba(53,85,230,.6)' }}>{listLabel}</button>
      </div>
    </Overlay>
  )
}
