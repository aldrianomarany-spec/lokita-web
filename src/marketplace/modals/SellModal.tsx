...
export default function SellModal() {
  ...
  // DONATIONS choose how the item reaches the receiver: keep it (direct) or
  // desk custody. PAID items always go through the LOKITA desk (0039).
  ...
  const effMode: 'desk' | 'direct' = s.giveawayOn ? mode : 'desk'
  ...
  // ---- posted ✓ — DIRECT donation: under review, the giver keeps the item ----
  if (s.listState === 'done' && effMode === 'direct') {
    return (
      <Overlay onClose={closeSell}>
        <div onClick={stop} style={{ background: '#FFFFFF', borderRadius: 0, padding: '34px 32px', width: '100%', maxWidth: 460, animation: 'lok-pop .26s cubic-bezier(.2,.8,.3,1) both', boxShadow: '0 40px 90px -20px rgba(0,0,0,.5)', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 13.5, color: '#4A4B4E', fontWeight: 500, lineHeight: 1.6, margin: '0 0 16px' }}>
            {t('The LOKITA team checks your post and it goes live right after — you’ll get a notification. You keep the item; neighbours who want it will chat you to arrange the handover.')}
          </p>
    ...
    <Overlay onClose={closeSell}>
      <div onClick={stop} style={{ background: '#FFFFFF', borderRadius: 0, padding: '30px 32px', width: '100%', maxWidth: 500, animation: 'lok-pop .26s cubic-bezier(.2,.8,.3,1) both', boxShadow: '0 40px 90px -20px rgba(0,0,0,.5)', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <p style={{ fontSize: 13, color: '#8B8B86', fontWeight: 600, margin: '0 0 18px' }}>
          📦 {s.giveawayOn
            ? t('How it works: the team reviews every donation post first. You choose below — keep the item and hand it over yourself, or drop it at the LOKITA desk.')
        ...
        {/* DONATIONS only: keep the item (direct) or desk custody — both are
            reviewed by the team first. Paid items are always desk consignment. */}
        {s.giveawayOn && (
          <>
