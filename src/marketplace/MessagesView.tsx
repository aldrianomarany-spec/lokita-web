          ...
          <div style={{ flex: 1, background: '#FFFFFF', border: '1px solid #D8D8D4', borderRadius: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 10, background: '#F5F5F3' }}>
              ...
              {s.msgs.map((m) => {
                ...
                return (
                  <div key={m.id} style={{ alignSelf: mine ? 'flex-end' : 'flex-start', maxWidth: '64%', background: mine ? 'var(--accent,#2A5FA8)' : '#FFFFFF', color: mine ? '#F7F3EA' : '#17181A', fontSize: 13.5, lineHeight: 1.45, padding: '10px 14px', borderRadius: mine ? '16px 16px 4px 16px' : '16px 16px 16px 4px', fontWeight: 500, boxShadow: '0 1px 2px rgba(18,19,21,.05)' }}>{m.content}</div>
                )
              })}
            </div>
            <div style={{ padding: '14px 18px', borderTop: '1px solid #E6E6E3', display: 'flex', gap: 10, alignItems: 'center' }}>
              ...
              />
              <button onClick={() => sendMsg()} className="lok-btn" style={{ border: 'none', background: 'var(--accent,#2A5FA8)', color: '#F7F3EA', fontFamily: 'inherit', fontWeight: 700, fontSize: 13.5, padding: '12px 20px', borderRadius: 0, cursor: 'pointer' }}>Send</button>
            </div>
          </div>
          )}
        </div>
