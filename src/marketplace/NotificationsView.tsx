...
const TYPE_META: Record<NotifType, TypeMeta> = {
  ...
  order_update: { typeLabel: 'ORDER', filterLabel: 'Order Updates', iconBg: '#E7F1EA', iconFg: '#1B7A4B', icon: s2(<><path d="M12 2 4 6v6c0 5 3.4 8.2 8 10 4.6-1.8 8-5 8-10V6z" /><path d="M9 12l2 2 4-4" /></>) },
    ...
    <div style={{ animation: 'lok-fade .3s ease both', maxWidth: 760, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 20 }}>
        {FILTERS.map(([key, label]) => {
          ...
          return (
            <button key={key} onClick={() => selectNotifFilter(key)} className="lok-chip" style={{ cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600, fontSize: 12.5, padding: '8px 13px', borderRadius: 20, border: `1px solid ${active ? '#201E18' : '#E4DDCE'}`, background: active ? '#201E18' : '#FBF8F1', color: active ? '#F7F3EA' : '#4A463B', display: 'flex', alignItems: 'center', gap: 6 }}>
              {label}
      ...
      {s.notifsLoading && s.notifs.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {filtered.map((n) => {
            ...
            const u = unread(n)
            return (
              <div key={n.id} onClick={() => openNotifTarget(n)} className="lok-btn" style={{ cursor: 'pointer', display: 'flex', gap: 14, alignItems: 'flex-start', background: u ? '#FBF8F1' : '#F5F0E6', border: `1px solid ${u ? '#D8CFBB' : '#EEE7D8'}`, borderRadius: 16, padding: '15px 17px' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {n.body && <div style={{ fontSize: 13, color: '#6F6A5C', lineHeight: 1.45, marginTop: 3 }}>{n.body}</div>}
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8, flex: 'none' }}>
                  <span style={{ fontFamily: "'Spline Sans Mono',monospace", fontSize: 10, color: '#A29C8B' }}>{timeAgo(n.created_at)}</span>
