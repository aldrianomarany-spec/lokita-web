...
export default function ListingCard({ it, index }: { it: EnrichedItem; index: number }) {
  ...
  return (
    ...
    >
      <div style={{ position: 'relative', height: 172, overflow: 'hidden', background: tn.tint }}>
        {it.photoUrl ? (
          <img className="lok-img" src={it.photoUrl} alt={it.title} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }} />
        ) : (
          <div
            className="lok-img"
            style={{ position: 'absolute', inset: 0, backgroundImage: `repeating-linear-gradient(135deg,${tn.stripe} 0 12px,transparent 12px 24px)`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          >
            <span style={{ fontFamily: "'Spline Sans Mono',monospace", fontSize: 11, color: tn.label, letterSpacing: '.02em', textAlign: 'center', padding: '0 16px' }}>{it.photo}</span>
          </div>
        )}
        {tc && (
          <span style={{ position: 'absolute', top: 11, left: 11, fontFamily: "'Spline Sans Mono',monospace", fontSize: 9.5, fontWeight: 600, color: tc.fg, background: tc.bg, padding: '4px 8px', borderRadius: 0, letterSpacing: '.04em' }}>{t(it.tag)}</span>
        )}
        <button
          onClick={(e) => {
            e.stopPropagation()
      ...
      <div style={{ padding: '14px 15px 15px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingTop: 11, borderTop: '1px solid #E6E6E3' }}>
          <div style={{ width: 24, height: 24, borderRadius: '50%', background: tn.tint, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 11, color: '#1E1E1E', flex: 'none' }}>{it.sellerInitial}</div>
